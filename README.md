...
* Вам нужно разработать собственный CI/CD-конвейер автоматизации для проекта машинного обучения с использованием **Docker и локального GitLab**. В ходе выполнения практического задания вам необходимо автоматизировать сбор данных, подготовку датасета, обучение модели и работу модели.
* Разработанный конвейер описывается в файле `.gitlab-ci.yml` в корне проекта. Все скрипты этапов конвейера требуется сохранить.
* Все файлы необходимо разместить в подкаталоге lab2 корневого каталога
* Шаблон инфраструктуры (compose-файл с GitLab CE + Runner, линтер `.gitlab-ci.yml`, пример пайплайна и пошаговая инструкция со скриншотами) лежит в `lab2/` — см. `lab2/README.md`.

Этапы задания

1. Поднять локально GitLab CE и GitLab Runner через Docker (`lab2/gitlab-compose.yml`), создать проект, зарегистрировать runner.
...
5. Загрузить сохранённую модель на предыдущем этапе и проанализировать её качество на тестовых данных.
6. Описать конвейер в `.gitlab-ci.yml` минимум с тремя обязательными стейджами — `prepare-dataset`, `train`, `test` — и обеспечить прохождение линтера (`lab2/ci_lint.py`). Связать конвейер с системой контроля версий (push в локальный GitLab → автозапуск pipeline).
...
2. Проект оркестируется с помощью CI/CD (GitLab CI или GitHub Actions).
